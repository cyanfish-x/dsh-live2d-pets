...
- 设置项（六项）：
  ...
  - **开发者选项**（独立分组）：总开关 `developerMode`（默认关）；开启后显示**调试面板**开关（`debug`，默认关）与**显示点击分区**（`showTapZones`，默认关；与调试面板可独立开关），并在「内置模型（只读）」标题右侧显示「打开内置模型配置文件」按钮（点击弹层展示 `src/presets/presets.jsonc` 路径供复制；DSH 0.1.5 起 openPath 能力已移除，弹层为唯一路径，ADR-011）
- **自定义人设（人设区工具行）**：
  - 「↻ 重新读取」：Host 现读人设文件 + SSE 推送，宠物与下拉**当场更新**（不重启不刷新；文件改坏时保留上一份好配置并在页面提示）
  - 「自定义人设 ↗」：弹层展示 `$DSH_HOME/live2d-pet/personas.jsonc` 路径（一键复制文件路径 + 一键复制女仆模板）；若 DSH 提供 openPath 能力（0.1.0 时代，0.1.5 起已移除）则优先直达打开（ADR-011）
...
- **人设体系**（ADR-007）：
  ...
  - 「自定义人设 ↗」client 侧结构探测 `ctx.get('workspaces').openPath`：存在则直达打开，0.1.5 起 IWorkspaces 无此方法 → 弹层兜底（复制路径/模板，ADR-011）
